...
def diagnose_d4():
    ...
    pq = PolarAngleQuantizer(d=D) # L=2
    ...
    for i in range(len(pa_tr)):
        print(f"Level {i} Angle Chunk Diff: {(pa_tr[i].view(-1).to(torch.int32) - pq.pack_all(pq.quantize_all(angs_py))[i].view(-1).to(torch.int32)).abs().max().item()}")

    print(f"\n--- D={D} DECODER CHECK ---")
    # PY rec from PY packed
    ...
    if cos_sim < 0.99:
        print(f"PY: {x_rec_py.view(-1)}")
        print(f"TR: {x_rec_tr.view(-1)}")
